/**
 * OpenTelemetry SDK bootstrap for socketServer (Express/Node.js).
 * MUST be imported at the very top of index.ts, before any other modules,
 * so that auto-instrumentation patches are applied before they load.
 *
 * OTLP (Jaeger/Grafana) → set OTEL_EXPORTER_OTLP_ENDPOINT in .env
 * Console fallback       → leave OTEL_EXPORTER_OTLP_ENDPOINT blank
 */

import "dotenv/config"; // load .env before reading any env vars
...
export function startTracing() {
  // Read env vars here — after dotenv/config has loaded .env
  const otlpEndpoint = process.env.OTEL_EXPORTER_OTLP_ENDPOINT;
  const isProduction = process.env.NODE_ENV === "production";

  // In production with no OTLP endpoint → skip tracing to avoid console spam
  ...
  const resource = resourceFromAttributes({
    [ATTR_SERVICE_NAME]: "rydex-socket-server",
